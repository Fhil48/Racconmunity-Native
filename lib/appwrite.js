...
import { formatDateForAppwrite, getStartAndEndOfMonth, getStartAndEndOfWeek } from "./helpers";
...
export async function uploadFile(file, type, category) {
  ...
  try {
    console.log('id bucket',appwriteConfig.storageId)
    const uploadedFile = await storage.createFile(
      appwriteConfig.storageId[category],
      ID.unique(),
      asset,
    );
...
export async function changeStatusEvent(id, type) {
  try {
    ...
    return { event: eventDocument, message: `El evento ${type === 'canceled' ? 'ha sido cancelado.' : 'ha finalizado'}`};
  } catch (error) {
    throw new Error(error.message);
  }
}
