...
export async function editEvent(form) {
  ...
  try {
    ...
    if (event.thumbnail) {
      ...
      await storage.deleteFile(appwriteConfig.eventStorageId, previousThumbnailId);
    }
...
export async function getNearestEvent() {
  try {
    ...
    console.log("currentDateTime local:", currentDateTime.toLocaleString()); 

    // Convertimos currentDateTime a formato UTC
    const currentDateTimeUTC = currentDateTime.toISOString();
    console.log("currentDateTime UTC:", currentDateTimeUTC); 

    const dateOnly = currentDateTimeUTC.split("T")[0];
    ...
    const posts = await databases.listDocuments(
      ...
      [
        ...
        Query.lessThanEqual("date", endTime.toISOString()),
