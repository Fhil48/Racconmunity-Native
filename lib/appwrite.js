...
export async function uploadFile2(file, type, category) {
  console.log(file);
  if (!file) return;
  ...
  try {
    ...
    // Crear el objeto asset con los datos del archivo
    const asset = {
      type: mimeType,
      name: file.name,
      size: file.size,
    ...
    console.log("storageCategory", storageId[category]);
    ...
    // Obtener la URL del archivo subido (asumiendo que getFilePreview devuelve la URL)
    const fileUrl = await getFilePreview(uploadedFile.$id, "image", category);
...
export async function changeStatusEvent(id, type) {
  try {
    ...
    // events -> user many to one
    console.log("evento encontrado: ", eventDocument);
    console.log("Type: ", type);
    console.log("usuario actual: ", actualUser.$id);

    const validStates = ['active', 'canceled', 'finished'];
